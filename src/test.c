...
int main(int argc, char** argv) {
	...
	if(to_u32(entry.key) != 10 || to_u32(entry.data) != 100) {
		printf("remove return value is not corrent: %lu: %lu, expected: %u: %u " FAIL, (uintptr_t)entry.key, (uintptr_t)entry.data, 10, 100);
	}
	...
	if(to_u32(entry.key) != 30 || to_u32(entry.data) != 300) {
		printf("remove return value is not corrent: %lu: %lu, expected: %u: %u " FAIL, (uintptr_t)entry.key, (uintptr_t)entry.data, 30, 300);
	}
